...
impl<T> Node<T> {
    /// Balance a [`Node`] whose right branch has greater height than the left.
    ///
    /// # Errors
    /// Makes no changes if `self`
    ///
    /// * has a left child
    /// * does not have a right child
    /// * the right child has a left child
    /// * the right child does not itself have a right child
    ///
    /// # Performance
    ...
    /// assert!(root.left.as_deref().is_some_and(|left| left.element == 0));
    /// assert!(root.right.as_deref().is_some_and(|right| right.element == 2));
    /// ```
    fn rotate_left(&mut self) -> Result<(), ()> {
        ...
        let Some(child) = self.right.as_deref_mut() else {
            return Err(());
        };

        let Some(grandchild) = child.right.as_deref_mut() else {
            return Err(());
        };
        ...
        core::mem::swap(&mut self.element, &mut child.element);
